...
const AccessForm = () => {
  ...
  const [visitors, setVisitors] = useState([
    { name: "", mobile: "", aadhar: "", materials: "" }, // default visitor 1
  ]);

  const addVisitor = () => {
    setVisitors((prev) => [
      ...prev,
      { name: "", mobile: "", aadhar: "", materials: "" },
    ]);
  };
  ...
  const handleSubmit = (e) => {
    ...
    localStorage.setItem(
      "accessRequest",
      JSON.stringify({ requester, visitors })
    );
            ...
            <section className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  onClick={addVisitor}
                  required
                  className="inline-flex cursor-pointer items-center gap-2 px-3 py-1.5 bg-sky-600 hover:bg-sky-700 text-white rounded-lg text-sm shadow"
                >
                  + Add Visitor
                </button>
              </div>
                    ...
                    <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-3">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          required
                          maxLength={10}
                          onChange={(e) => handleVisitorChange(idx, "mobile", e.target.value.replace(/\D/, ""))}
                          className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 outline-none focus:border-sky-500"
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          maxLength={12}
                          pattern="\d*"
                          onChange={(e) => handleVisitorChange(idx, "aadhar", e.target.value.replace(/\D/, ""))}
                          className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 outline-none focus:border-sky-500"
                        />
                      </div>
                      ...
                      <div className="md:col-span-2">
                        <label className="block text-sm font-medium text-slate-600 mb-1">
                         <span className="text-red-500">* </span> Returnable Materials (e.g., Laptop, Tools)
                        </label>
                        <input
                          type="text"
                          value={v.materials}
                          required
                          onChange={(e) => handleVisitorChange(idx, "materials", e.target.value)}
                          className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 outline-none focus:border-sky-500"
                        />
                      </div>
                    </div>
